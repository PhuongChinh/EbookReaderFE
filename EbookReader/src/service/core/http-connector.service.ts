...
export class HttpConnectorService {
  ...
  apiUrlOther = CONSUME_API.apiUrlOther
  headers = {
    jsondata: {
      'headers': {
  ...
  // get from workPress
  public _reqWP(method: string, uri: string,
    pl: any = undefined, options: any = undefined,
    isNotSilenceLoad: boolean = true): Observable<any> {
    let url = this.supportIE(this.apiUrlOther + uri, this.apiUrlOther)
  ...
  public getWP(uri: string, filter: any = undefined): Observable<any> {
    if (typeof filter == 'object') {
      let params = this.buildBodyParam(filter);
      if (uri.lastIndexOf('?') > 0) {
        uri = uri + '&' + params;
  ...
  public post(uri: string, payload: any, options: any = null) {
    return this._req('post', uri, payload, options);
  }
